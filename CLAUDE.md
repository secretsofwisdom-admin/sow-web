...
- **Tokens are three `static const` instances behind a Riverpod provider**, not a `ThemeExtension`. `_WheelPainter` has no `BuildContext` at paint time so it needs the palette passed as a field regardless, which is what the provider approach already does; `ThemeExtension` would also demand a hand-written `lerp` over 16 fields for a crossfade nobody wants under a chart wheel. Providers live in `lib/providers/theme_providers.dart` and are named to match BaZi's (`themeIdProvider`, `imperialTokensProvider`, `themeDataProvider`).
  ...
  - `imperial`'s `errorInk` measures **3.66:1** on its own `panelFill` — under AA for the 12.5px error text it paints. Pre-existing and preserved because imperial is frozen; the test pins it at 3.6 so it cannot get worse. `lifted` does not inherit the shortfall.
...
Three palettes, added 2026-08-21: `imperial` (the original), `lifted` (dark, raised grounds) and `light` (warm parchment). Selectable from a nav select on all 11 nav-bearing pages and from a `PopupMenuButton` inside the astro calculator.

- **`imperial` is frozen and is the *absence* of the attribute.** The default lives in bare `:root`; the other two are `html[data-theme="lifted"|"light"]` blocks, which at (0,1,1) beat `:root` at (0,1,0) with no `!important`. A visitor with nothing stored gets byte-identical CSS to before the feature existed. `theme_tokens_test.dart` pins all 16 app-side values; don't "tidy" them.
...
- **Light `--galaxy` is `.14`, not `.10`.** The imperial ceiling does not transfer: in light the veil *darkens*, so the constraint flips sign. Measured against the 12px footer line in light `--ink-muted` over `--bg-mid`: `.10` → 5.10, `.14` → 4.61, `.16` → 4.38, `.18` → 4.19. `.16` is the ceiling. `lifted` inherits `.10` and deliberately does not redeclare it.
...
site → app   { source:'sow',      type:'theme', theme:'imperial'|'lifted'|'light' }
...
## TODO
- **BaZi is still dark-only.** It ships five themes (`imperial`/`classic`/`crimson`/`jade`/`porcelain`) with its own switcher and `SharedPreferences`, and ignores the theme channel, so `calculator.html` stays imperial while the site around it goes light. Reducing it to the same three needs **60 hand-authored element colours** (5 families × 6 roles × 2 new palettes) — the largest remaining palette task, and invisible from the astro side. Delete `loadSavedTheme()` from its `main.dart` when wiring the channel (see the race note above).
- **Light mode has no backdrop image** — `html[data-theme="light"] body::after` is `content: none`. The intended asset is `celestial-veil-light.jpg`: the existing veil **inverted, desaturated, then re-tinted warm** via `ImageOps.colorize(gray, black='#4A3E28', white='#FFFFFF')`, composited with `multiply`. Naive inversion is hue-wrong (measured G−B **+9.7**, a green cast that multiply drags into the parchment — the exact failure `screen` was chosen to avoid). Inversion is per-pixel so the dead-centre medallion crop survives; assert 1280×696 after generating. Two copies to keep in sync (site `images/`, `AstroCalculator/assets/images/`) — **BaZi has no veil and needs none**.
...
scripts/          - check_theme_invariants.py (read-only; run from repo root)
